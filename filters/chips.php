...
<?php if( $chips ) : ?>
    ...
    <li class="wpc-filter-chip <?php echo esc_attr( $chip['class'] ); ?>">
        <a href="<?php echo esc_url( $chip['link'] ); ?>" title="<?php if( $chip['name'] !== esc_html__('Очистити фільтри', 'filter-everything') ){
            if ( $chip['class'] === 'wpc-chip-search' ) {
                echo esc_attr( sprintf( __('Видалити %s з результатів пошуку', 'filter-everything'), '&laquo;'.$chip['label'].'&raquo;' ) );
            } else {
                echo esc_attr( sprintf( __('Видалити %s з результатів', 'filter-everything'), '&laquo;'.$chip['label'] .': '.$chip['name'].'&raquo;' ) );
            }
        } ?>">
            <span class="wpc-chip-content">
                <?php if (esc_html( $chip['name'] ) != "Reset all") {?>
					<span class="wpc-filter-chip-name"><?php echo esc_html( $chip['name'] ); ?></span>
				<?php } else {?>
					<span class="wpc-filter-chip-name">Очистити всі фільтри</span>
				<?php } ?>	
