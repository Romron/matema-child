...
.perelinking a {color: #FF6943;text-decoration: underline;}
.perelinking .single-flex-block {text-align: center;}
.perelinking .two-blocks-flex {align-items: flex-start;}
...
<div class="switchblock-section">
    <div class="container">
	<?php if ( is_page_template( 'page-switchblock.php' ) ) {} else {?>
...
videoId+'?rel=0&controls=1&showinfo=0&autoplay=1&start=0';var figure=element.querySelector('figure');var iframe=element.querySelector('iframe');element.classList.add('active');iframe.src=embedUrl;videoActive=element;}</script>
		...
		<?php if(get_field('perelinkovka_zagolovok_1')) {?>
		<div class="row margbot60 perelinking">
			<div class="two-blocks-flex">
				<div class="single-flex-block">
					<div><h3><?php the_field( 'perelinkovka_zagolovok_1' ); ?></h3>
						<?php the_field( 'perelinkovka_kolonka_1' ); ?>
					</div>
				</div>
				<div class="single-flex-block">
					<div><h3><?php the_field( 'perelinkovka_zagolovok_2' ); ?></h3>
						<?php the_field( 'perelinkovka_kolonka_2' ); ?>
					</div>
				</div>
			</div>
		</div>
